...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Hawkkit database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Hawkkit datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Hawkkit datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Hawkkit 0.7 datadir."
    ;;
  3)
    echo "Detected Hawkkit pre-0.8 datadir."
    ;;
  4)
    echo "Detected Hawkkit 0.8 datadir."
    ;;
esac
