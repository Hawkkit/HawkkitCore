...
# hawkkit_hash
RUN git clone https://github.com/hawkkitpay/hawkkit_hash
RUN cd hawkkit_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} hawkkit
RUN useradd -u ${USER_ID} -g hawkkit -s /bin/bash -m -d /hawkkit hawkkit
...
RUN mkdir /hawkkit-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /hawkkit-src && \
