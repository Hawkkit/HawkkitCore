...
// which are part of the Hawkkit repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="hawkkit-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /hawkkit-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "hawkkitpay-hawkkit/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from hawkkitpay-hawkkit/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'hawkkitpay-hawkkit/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from hawkkitpay-hawkkit/develop"
            }
          }

          if (hasCache) {
            sh "cd /hawkkit-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /hawkkit-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /hawkkit-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /hawkkit-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /hawkkit-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /hawkkit-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /hawkkit-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
