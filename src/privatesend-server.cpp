...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Hawkkit related functionality
...
/*
    ...
    allow endless transaction that would bloat Hawkkit and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Hawkkit specific functionality
