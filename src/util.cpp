...
const char * const BITCOIN_CONF_FILENAME = "hawkkit.conf";
const char * const BITCOIN_PID_FILENAME = "hawkkitd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "hawkkit" is a composite category enabling all Hawkkit-related debug output
                if(ptrCategory->count(std::string("hawkkit"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Hawkkit
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Hawkkit
    // Mac: ~/Library/Application Support/Hawkkit
    // Unix: ~/.hawkkit
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Hawkkit";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Hawkkit";
#else
    // Unix
    return pathRet / ".hawkkit";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty hawkkit.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override hawkkit.conf
